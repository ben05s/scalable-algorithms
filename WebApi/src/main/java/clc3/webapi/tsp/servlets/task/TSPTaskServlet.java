...
@WebServlet(urlPatterns = "/tsp/task/list")
public class TSPTaskServlet extends BaseServlet {
    TSPTaskDao taskDao = serviceLocator.getTspTaskDao();

    @Override
    protected void process(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        List<TSPTaskResponse> respTasks = new ArrayList<>();
        for (TSPTask task : taskDao.getAllOrderByCreated()) {
            ...
            respTasks.add(respTask);
        }
        jsonResponse(response, respTasks);
    }
}